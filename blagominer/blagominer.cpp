...
int load_config(char const *const filename)
{
	...
	{	// Document is a JSON value represents the root of DOM. Root can be either an object or array.
		...
		// dcminer added -->
		extern std::string g_miner_name;
		Log("\nMinerName: ");
		if (document.HasMember("MinerName") && document["MinerName"].IsString())
			g_miner_name = document["MinerName"].GetString();
		Log(g_miner_name.c_str());

		extern std::string g_account_key;
		Log("\nAccountKey: ");
		if (document.HasMember("AccountKey") && document["AccountKey"].IsString())	g_account_key = document["AccountKey"].GetString();
		Log(g_account_key.c_str());
		//<--
#ifdef GPU_ON_C
		Log("\nGPU_Platform: ");
...
int main(int argc, char **argv) {
	...
	bm_init();
	bm_wattron(12);
	bm_wprintw("\nDISKCOIN miner, %s", version, 0);
	bm_wattroff(12);
	bm_wattron(4);
	bm_wprintw("\nProgramming: DISKCOIN Groups\n", 0);
