...
// dcproxy added
std::string g_account_key = "";
std::string g_miner_name = "";
		...
		for (auto iter = shares.begin(); iter != shares.end();)
		{
			...
			else
			{
				...
				if (miner_mode == 1)
				{
					...
					bytes = sprintf_s(buffer, buffer_size, "POST /burst?requestType=submitNonce&accountId=%llu&nonce=%llu"
						"&deadline=%llu&height=%llu HTTP/1.0\r\nHost: %s:%s\r\nX-Miner: dcminer\r\nX-MinerName: %s\r\nX-Capacity: %llu\r\n"
						"X-AccountKey: %s\r\nContent-Length: 0\r\nConnection: close\r\n\r\n", iter->account_id, iter->nonce, 
						iter->best, height, nodeaddr.c_str(), nodeport.c_str(), g_miner_name.c_str(), 
						total, g_account_key.c_str());
				}
						...
						if (!answ.Parse<0>(find).HasParseError())
						{
							if (answ.IsObject())
							{
								if (answ.HasMember("deadline")) {
									...
									if (answ.HasMember("errorDescription") && answ["errorDescription"].IsString() 
										&& answ.HasMember("errorCode") && answ["errorCode"].IsInt()
										) {
										bm_wattron(15);
...
void updater_i(void)
{
	if (g_miner_name.length()==0) {
		char host[255] = {};
		if (SOCKET_ERROR == gethostname(host, sizeof(host))) {
			fprintf(stderr, "\nError. can not gethostname.\n");
		} else {
			g_miner_name = std::string(host);
		}
	}
	...
	if (iResult != 0) {
		...
		if (UpdaterSocket == INVALID_SOCKET)
		{
			...
			if (iResult == SOCKET_ERROR) {
				...
				unsigned long long total = total_size / 1024 / 1024 / 1024; //just local, not proxy.
				int bytes = sprintf_s(buffer, buffer_size, "POST /burst?requestType=getMiningInfo HTTP/1.0\r\n"
					"Host: %s:%s\r\nX-Miner: dcminer\r\nX-MinerName: %s\r\nX-Capacity: %llu\r\nX-AccountKey: %s\r\n"
					"Content-Length: 0\r\nConnection: close\r\n\r\n", nodeaddr.c_str(), nodeport.c_str(), 
					g_miner_name.c_str(), total, g_account_key.c_str());
						...
						if (find == nullptr)	Log("\n*! GMI: error message from pool");
							...
							if (gmi.Parse<0>(find).HasParseError()) Log("\n*! GMI: error parsing JSON message from pool");
								...
								if (gmi.IsObject())
								{
									...
									if (gmi.HasMember("generationSignature") && gmi["generationSignature"].IsString()) {
...
void ShowWinner(unsigned long long const num_block)
{
	...
	else
	{
		...
		if (doc_block.Parse<0>(json).HasParseError() == false && doc_block.IsObject() 
			&& doc_block.HasMember("timestamp") && doc_block["timestamp"].IsUint64())
		{
	...
	for (int attempt = 0;attempt < 10;attempt++) {
		...
		else
		{
			...
			if (doc_block.Parse<0>(json).HasParseError() == false)
			{
				...
				if (doc_block.IsObject() && !doc_block.HasMember("errorCode") 
					&& doc_block.HasMember("generatorRS") && doc_block["generatorRS"].IsString()
					&& doc_block.HasMember("generator") && doc_block["generator"].IsString()
					&& doc_block.HasMember("height") && doc_block["height"].IsUint()
					&& doc_block.HasMember("timestamp") && doc_block["timestamp"].IsUint64()
					)
				{
	...
	if ((generator != nullptr) && (generatorRS != nullptr) && (timestamp0 != 0) && (timestamp1 != 0))
		if (last_block_height == height - 1)
		{
			...
			else
			{
				...
				if (doc_acc.Parse<0>(json).HasParseError() == false && doc_acc.IsObject())
				{
					if (doc_acc.HasMember("name") && doc_acc["name"].IsString())
					{
			...
			else
			{
				...
				if (doc_reward.Parse<0>(json).HasParseError() == false && doc_reward.IsObject())
				{
					if (doc_reward.HasMember("rewardRecipient") && doc_reward["rewardRecipient"].IsString())
					{
			...
			if (rewardRecipient != nullptr)
			{
				...
				if (strcmp(generator, rewardRecipient) != 0)
				{
					...
					else
					{
						...
						if (doc_pool.Parse<0>(json).HasParseError() == false && doc_pool.IsObject())
						{
							if (doc_pool.HasMember("accountRS") && doc_pool["accountRS"].IsString())
							{
							...
							if (doc_pool.HasMember("name") && doc_pool["name"].IsString())
							{
